// calculations.js - Energy, stats, smoothing, alerts
...
function displayStats(data) {
    const voltages = data.map(d => d.voltage).filter(v => v !== undefined && !isNaN(v));
    const currents = data.map(d => d.current).filter(v => v !== undefined && !isNaN(v));
    const socs = data.map(d => d.soc).filter(v => v !== undefined && !isNaN(v));
    const powers = data.map(d => d.power).filter(v => v !== undefined && !isNaN(v));
    ...
    const voltageStatus = minVoltage < thresholds.voltageLow ? 'danger' : 
    ...
    const stats = [
        ...
        { label: 'Data Rate', value: data.length > 1 ? (data.length / (data[data.length-1].relativeTime || 1)).toFixed(1) + ' Hz' : 'N/A' }
    ];
